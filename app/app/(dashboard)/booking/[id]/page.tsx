...
export default function BookingPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="flex flex-col h-full md:block">
      {/* ── Mobile: each step is fullscreen with scroll ── */}
      <div className="flex-1 overflow-y-auto touch-scroll md:max-w-2xl md:mx-auto px-4 py-6">
        {/* Back */}
      ...
      {/* ── Footer actions — fixed at bottom on mobile, inline on desktop ── */}
      <div className="flex-shrink-0 bg-white border-t border-gray-200 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] md:pb-3 md:static md:mt-8 md:border-0 md:p-0 md:max-w-2xl md:mx-auto">
        <div className="flex gap-3">
          {step > 0 && (
            <button
              onClick={() => setStep(step - 1)}
              className="flex-1 py-3.5 rounded-xl border border-gray-200 text-gray-700 font-semibold hover:bg-gray-50 transition-colors text-sm active:scale-[0.98]"
            >
              Volver
            </button>
          )}
          {step < STEPS.length - 1 ? (
            <button
              ...
              className="flex-1 py-3.5 rounded-xl text-white font-semibold disabled:opacity-50 transition-opacity hover:opacity-90 text-sm active:scale-[0.98]"
              style={{ background: 'linear-gradient(to right, #7C3AED, #F97316)' }}
            >
              Siguiente
            </button>
            ...
            <button
              ...
              className="flex-1 py-3.5 rounded-xl text-white font-semibold transition-opacity hover:opacity-90 text-sm active:scale-[0.98]"
              style={{ background: 'linear-gradient(to right, #7C3AED, #F97316)' }}
            >
              Confirmar reserva
            </button>
